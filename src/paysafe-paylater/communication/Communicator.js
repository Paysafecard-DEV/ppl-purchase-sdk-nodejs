...
class Communicator {
    ...
    /**
     * Execute with an AuthorizationHeader
     * @param method "GET", "POST" or "PUT" http method to use
     * @param uri path to request
     * @param headerParams parameters to be sent in http headers
     * @param bodyParam the body to send (javascript Object, converted to Json)
     * @returns On success: a Promise with .data, .status, .headers, and .authorization. On error: an AxiosError
     */
    executeWithAuthorizationHeader(method, uri, headerParams, bodyParam) {
        return this.connection.request(method, uri, headerParams, bodyParam).then(({ data, status, headers }) => ({
            data,
            status,
            headers,
            authorization: this.extractAuth(headers),
        }));
    }

    extractAuth(headers) {
        if (headers.hasOwnProperty("access_token")) {
            return "Bearer " + headers["access_token"];
        } else {
            return null;
        }
    }
